...
# dpr Release Script
# ============================================================================
# Prepares and publishes a new release of dpr
#
# Features:
# - Runs verification checks (lint, typecheck, tests, build)
# - Validates git repository is clean
# - Updates version in package.json
# - Creates git tag and pushes to remote
# - GitHub Actions handles cross-platform builds and npm publishing
#
# Usage: ./scripts/release.sh [options]
#
# Options:
#   --allow-uncommitted    Allow release with uncommitted git changes
#   --skip-verify          Skip lint/typecheck/tests checks
...
parse_args() {
    while [[ $# -gt 0 ]]; do
        ...
        -h | --help)
            ...
            echo "  --skip-verify          Skip lint/typecheck/tests checks"
...
preflight_checks() {
    ...
    if [[ ! -f "$PROJECT_ROOT/package.json" ]]; then
        error "Must be run from the project root directory (package.json not found)"
        exit 1
    fi
...
run_verification() {
    ...
    info "Running full validation (typecheck, lint, format, tests)..."
...
update_package_version() {
    step "Step 5: Updating package.json version..."
    divider

    cd "$PROJECT_ROOT"

    if [[ "$DRY_RUN" == true ]]; then
        info "[DRY-RUN] Would update version in package.json from $CURRENT_VERSION to $NEW_VERSION"
        return 0
    fi

    # Use npm version to update (without creating git tag - we'll do that ourselves)
...
# ============================================================================
git_commit_tag_push() {
    step "Step 6: Creating git commit and tag..."
    divider
    ...
    if [[ "$DRY_RUN" == true ]]; then
        ...
        echo "  - Stage: package.json package-lock.json"
...
main() {
    echo ""
    echo -e "${BOLD}${CYAN}======================================================${NC}"
    echo -e "${BOLD}${CYAN}              dpr Release Script                      ${NC}"
    echo -e "${BOLD}${CYAN}======================================================${NC}"
    echo ""
    ...
    if [[ "$DRY_RUN" == true ]]; then
        ...
        echo ""
        echo "  Monitor progress at: https://github.com/tx2z/dpr/actions"
    fi
    echo ""
    divider
}
